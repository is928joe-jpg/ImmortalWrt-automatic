...
echo "========================================="
echo "🍞 构建开始: $TARGET_ARCH | ${ROM_SIZE}MB"
echo "========================================="
...
case "$TARGET_ARCH" in
    "x86-64")        PROFILE="" ;;
    "rockchip-armv8")    PROFILE='PROFILE="friendlyarm_nanopi-r4s"' ;;
    "mediatek-filogic")  PROFILE='PROFILE="xiaomi_ax6000"' ;;
esac
...
if [ ${PIPESTATUS[0]} -ne 0 ]; then
    echo ""
    echo "❌ 构建失败:"
    grep -iE "error|failed|cannot find|not found|no such|No package" /tmp/build.log | tail -5
    exit 1
fi
